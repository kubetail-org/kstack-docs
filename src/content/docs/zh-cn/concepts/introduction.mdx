...
技能由纯 Markdown 文件和配套 shell 脚本组成。引导脚本（[https://kstack.sh/install](https://kstack.sh/install)）会检测 `PATH` 中每个受支持的代理并安装到其中：Claude Code、OpenAI Codex CLI、OpenCode、Cursor、Factory Droid、Slate、Kiro、Hermes。各代理的行为完全一致，仅安装路径不同。
