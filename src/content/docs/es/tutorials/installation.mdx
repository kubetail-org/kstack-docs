...
Las instalaciones locales reflejan esta estructura bajo el directorio del proyecto (p. ej. `<project>/.codex/skills/`). Pi es la única excepción: su directorio local de skills es `<project>/.pi/skills/` (sin el segmento `agent/`). Consulte [Uso de kstack con agentes distintos de Claude](/es/guides/non-claude-agents/) para notas por agente.
