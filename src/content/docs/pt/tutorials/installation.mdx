...
Instalações locais espelham essa estrutura no diretório do projeto (ex.: `<project>/.codex/skills/`). Pi é a única exceção — seu diretório local de skills é `<project>/.pi/skills/` (sem o segmento `agent/`). Veja [Usando kstack com agents que não são Claude](/pt/guides/non-claude-agents/) para notas por agent.
