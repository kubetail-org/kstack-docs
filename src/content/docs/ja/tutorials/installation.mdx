...
ローカルインストールはプロジェクトディレクトリ以下にこの構造を反映します（例：`<project>/.codex/skills/`）。Pi だけは例外で、ローカルの skills ディレクトリは `<project>/.pi/skills/` です（`agent/` セグメントなし）。エージェントごとの注意事項については[Claude以外のエージェントでのkstackの使用](/ja/guides/non-claude-agents/)を参照してください。
